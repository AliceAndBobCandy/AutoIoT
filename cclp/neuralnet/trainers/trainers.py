...
class Trainer(object):
    ...
    def __init__(self, params, net_model, t_sup_labels):
        ...
        self._loss_logit_weighted = self._get_loss_logit_weighted(net_model) #根据step动态调整loss的权重
        self._cclp_weight = self._get_cclp_weight(net_model)
        ...
        self._model_step_now = net_model.get_t_step()
    ...
    def _setup_losses(self, net_model, t_sup_labels):
        ...
        if self._params["dynamic_loss_weight"]==True:
            weight1 = self._loss_logit_weighted
            cclp_weight = self._cclp_weight
            tf.summary.scalar('logit_weight',self._loss_logit_weighted)
            tf.summary.scalar('cclp_weight',self._cclp_weight)
        else:
            weight1 = self._params["logit_weight"]
        ...
        if self._params["cc_loss_on"]:
            cclp_loss = losses.add_cclp_loss( # execute label propogation and compute loss of T and H
                ...
                cc_loss_distance = self._params["cc_loss_distance"],
                # Others
                e_smooth = self._params["cc_e_smooth"],
                optim_smooth_mtx = self._params["cc_optim_smooth_mtx"],
                dynamic_loss_weight = self._params["dynamic_loss_weight"]
                )
    ...
    def _get_loss_logit_weighted(self, net_model):# step: tf.Variable(0, trainable=False, name='global_step')
        logit_loss_weight = tf.minimum( tf.train.piecewise_constant( net_model.get_t_step(), boundaries = [ 500,1000,1200,1500,2500,3000,3500 ], values = [0.3,0.6,2.0,4.0,8.0,12.0,12.0,15.0]),15.0)           
        return logit_loss_weight
    def _get_cclp_weight(self, net_model):# step: tf.Variable(0, trainable=False, name='global_step')
        cclp_weight = tf.maximum( tf.train.piecewise_constant( net_model.get_t_step(), boundaries = [ 1000,1200,1500,3000,3500 ], values = [3.0,1.0,0.5,0.3,0.2,0.1]),0.3)           
        return cclp_weight
    ...
    def _create_train_op(self): # define optimizer and execute gradient descent for trainable variables.
