...
def warsserstein(T,H):# T and H are all N*N matrix
    pass
...
def regularize_for_compact_clustering( H_trans,
    ...
    T_trans = tf.matmul( Phi_lu, phi_div_mass, transpose_b=True, name='T_trans')#compute according to distance of LP
    ...
    # warsserstein loss between T_trans and H_step
    if cc_loss_distance=="warsserstein":   
        T_trans_np = T_trans.eval(tf.Session())  #!!!???
        print(T_trans_np)
        loss_step = warsserstein(T_trans, H_step)
    #-----------
    else:
        loss_step = tf.reduce_mean( - tf.reduce_sum( T_trans * tf.log( H_step + eps_log), axis=[1] ) ) #crossentropy loss
    tf.summary.scalar('Loss_step1', loss_step)
    # tf.summary.scaler('cclp_weight_per_chain',cclp_weight_per_chain)
    if cclp_steps == 1 or sum_over_chains :
        loss_cclp += loss_step
        
        if dynamic_loss_weight==True:
        ...
        # if tf.cond(tf.greater(tf.cast(cclp_weight_per_chain,tf.float32),tf.constant(0.0)),lambda:1,lambda:0):#!!!
                loss_step_weighted = tf.multiply( loss_step, cclp_weight_per_chain, name='loss_step1_weighted' )
                #LOG.debug("Adding loss. weight="+str(cclp_weight_per_chain))
                tf.losses.add_loss( loss_step_weighted )
...
def add_cclp_loss(Z_l,
    ...
    H_trans = make_graph_and_transition_mtx( Z_lu, sim_metric, l2_sigmas, l2_sigmas_trainable )#transition matrix computed according to distance
    ...
    regularize_for_compact_clustering( H_trans = H_trans_smooth if optim_smooth_mtx else H_trans,
                                       ...
                                       cc_loss_distance = cc_loss_distance,
                                       dynamic_loss_weight = dynamic_loss_weight )
    
#========================function used for original space similar====================
def get_Htrans_through_orignal_space(t_sup_images,t_unsup_images,e_smooth,uniform_p,sim_metric, l2_sigmas, l2_sigmas_trainable):#whether t_sup_images is list by class???
     F_lu = tf.concat( values=[ t_sup_images, t_unsup_images ], axis=0 )
     H_trans = make_graph_and_transition_mtx( F_lu, sim_metric, l2_sigmas, l2_sigmas_trainable )
     H_trans = tf.cast(H_trans,tf.float32)
     H_trans_smooth = e_smooth * uniform_p + (1.0 - e_smooth) * H_trans
     return H_trans_smooth
#====================================================================================    
    
#####################################################################
#            Standard Cross Entropy - for labelled data             #
#####################################################################
def generate_weights(onehot_labels,num_classes=25):
    # weights generate(label 24 has larger weight)
    weights = [1.0]*(num_classes-1) + [2.0]
    weights = tf.convert_to_tensor(weights)
    weights = tf.reduce_sum(tf.multiply(onehot_labels,weights),-1)   
    return weights
def add_softmax_cross_entr(logits, lbls, weight=1.0, lbl_smoothing=0.0,use_multi_task_model=0, logits2=None):
