...
class DataSet(object):
    def __init__(   self,
        ...
        if Y_generated is not None:
            assert images.shape[0] == Y_generated.shape[0]
        self._num_examples = images.shape[0]
        self._originalShapeOfAnImg = images.shape[1:]
        self._num_classes = int(np.max(labels) + 1)
        self._Y_generated = Y_generated
    ...
    def _update_stats(self, norm_type, zca_instance):
        ...
        if self._samples.ndim is 2:
            self._db_stats = {  "norm_type": norm_type,                           
                            "min": np.min(self._samples, axis=(0,1), keepdims=True)*1.,
                            "max": np.max(self._samples, axis=(0,1), keepdims=True)*1.,
                            "mean": np.mean(self._samples, axis=(0,1), keepdims=True, dtype="float32"),
                            "std": np.std(self._samples, axis=(0,1), keepdims=True, dtype="float32"),
                            }
        else:
            self._db_stats = {  "norm_type": norm_type,
    ...
    def labels(self):
        return self._labels
    @property
    def Y_generated(self):
        return self._Y_generated
    @property
    ...
    def get_samples_and_labels(self):
        if self._Y_generated is not None:
            return (self._samples, self._labels,self._Y_generated)
        else:
    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        if self._epochs_completed == 0 and start == 0 and shuffle:
            ...
            if self._val_on_clean_data:
                self._Y_generated = self.Y_generated[perm0]
        # Go to the next epoch
        if start + batch_size > self._num_examples: # not enough for the rest num_examples
            # Finished epoch
            ...
            if self._val_on_clean_data:
                Y_generated_rest_part = self._Y_generated[start:self._num_examples]
            # Shuffle the data
            if shuffle:
                ...
                if self._val_on_clean_data:
                    self._Y_generated = self.Y_generated[perm]
            # Start next epoch
            start = 0
            self._index_in_epoch = batch_size - rest_num_examples # get rest part and sample new part
            end = self._index_in_epoch
            images_new_part = self.samples[start:end]
            labels_new_part = self.labels[start:end]
            if self._val_on_clean_data:
                Y_generated_new_part = self.Y_generated[start:end]
                return np.concatenate((images_rest_part, images_new_part), axis=0) , np.concatenate((labels_rest_part, labels_new_part), axis=0),\
                                        np.concatenate((Y_generated_rest_part, Y_generated_new_part), axis=0)
            else:
                return np.concatenate((images_rest_part, images_new_part), axis=0) , np.concatenate((labels_rest_part, labels_new_part), axis=0)
        else: # in the middle, can get samples easily
            self._index_in_epoch += batch_size
            end = self._index_in_epoch
            if self._val_on_clean_data:
                return self._samples[start:end], self._labels[start:end],self._Y_generated[start:end]
            else:
